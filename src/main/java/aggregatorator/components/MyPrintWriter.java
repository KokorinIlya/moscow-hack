...
public class MyPrintWriter extends Writer {
	...
	public boolean checkError() {
		...
		if (out instanceof MyPrintWriter) {
			MyPrintWriter pw = (MyPrintWriter) out;
	...
	public void write(char buf[], int off, int len) {
		try {
			synchronized (lock) {
				...
				System.out.write(new String(buf).getBytes(), off, len);
			}
		}
		catch (InterruptedIOException x) {
			Thread.currentThread().interrupt();
	...
	public void write(String s, int off, int len) {
		try {
			synchronized (lock) {
				ensureOpen();
				out.write(s, off, len);
				System.out.write(s.getBytes(), off, len);
			}
		}
		catch (InterruptedIOException x) {
			Thread.currentThread().interrupt();
	...
	private void newLine() {
		try {
			synchronized (lock) {
				...
				System.out.write(lineSeparator.getBytes());
				if (autoFlush)
					out.flush();
			}
		}
